...
//methods
	...
	void 				merge_init();
	void 				merge_all();
	std::vector<int*> 	merge_set(int *ha, std::vector<int*> &K, z3::solver &z3solver);
	int* 				merge_AB(int* ha, int* hb, z3::solver &z3solver);
	void				redundancy_removal_all();
	void				redundancy_removal(int *bools, z3::solver &solver);
	
	//volume & barvinok routine
	const double 	call_barvinok() { 
						if (!islia()) err_logic_latte();
						return lat_count(bsols.back()) * multiplier.back(); };
	const double 	call_barvinok(const unsigned int index) { 
						if (!islia()) err_logic_latte();
						return lat_count(bsols[index]) * multiplier[index]; };
	const double 	call_alc() { 
						if (!islia()) err_logic_latte();
						return approx_lat_count(bsols.back()) * multiplier.back(); };
	const double 	call_alc(const unsigned int index) { 
						if (!islia()) err_logic_latte();
						return approx_lat_count(bsols[index]) * multiplier[index]; };
...
//methods
	...
	const double 	lat_count_light(int *bools, int var);
	const double 	lat_count_basic(int *bools, unsigned int nRows, std::vector<int> vars);
	const double 	approx_lat_count_basic(int *bools, unsigned int nRows, std::vector<int> vars);
	const bool 		lattice_feasible(int *bools, unsigned int nRows, std::vector<int> vars);
	const double 	lat_count(int *boolsol);
	const double 	approx_lat_count(int *boolsol);
