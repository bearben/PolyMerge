...
//// Lattice Counting ////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
const double volce::solver::lat_count_light(int *bools, int var){
...
const double volce::solver::lat_count_basic(int *bools, unsigned int nRows, std::vector<int> vars){
	//nVars: the number of "decided" numeric variables
	//nRows: the number of "decided" linear formulas
	//nFormulas: the number of linear formulas
	//bools[nFormulas], vars[nVars]
	...
	//counting
	std::string filename = "barvinok_in.tmp";
	...
	if (wordlength > 0){
		for (unsigned int i = 0; i < nVars; i++){
			...
			for (unsigned int j = 0; j < nVars; j++)
				ofile << ((i == j) ? "1 " : "0 ");
			ofile << pow(2, wordlength - 1) - 1 << std::endl;
			ofile << "1 ";
			for (unsigned int j = 0; j < nVars; j++)
				ofile << ((i == j) ? "-1 " : "0 ");
			ofile << pow(2, wordlength - 1) << std::endl;
		}
	}
	...
	for(unsigned int i = 0; i < nFormulas; i++) {
		...
		double b = round(bigb[i]);
		
		bool hasdecimal = (abs(1 - (double)b / bigb[i]) > 0.001);
		for (unsigned int j = 0; j < nVars; j++)
		{
			a[j] = round(bigA(i, vars[j]));
			if (abs(1 - (double)a[j] / bigA(i, vars[j])) > 0.001)
				hasdecimal = true;
		}
		if (hasdecimal)
		{
	...
	//execute latte
	std::string cmd = tooldir + "/barvinok_count < " + filename;
	int proc = system(cmd.c_str());
...
const double volce::solver::lat_count(int *boolsol){
	std::vector<int> vars;
	unsigned int nRows = 0;
	double count = 1;
	...
	if (!enable_fact){
		...
		if (nVars_decided_total == 0)
			...
			return lat_count_light(boolsol, vars.back()) * cube_count;
		else
			return lat_count_basic(boolsol, nRows, vars) * cube_count;
	}
	...
	for (unsigned int i = 0; i < npbools; i++){
		...
		if (nVars_decided == 1)
			count *= lat_count_light(pbools[i], vars.back());
		else
			count *= lat_count_basic(pbools[i], nRows, vars);
...
//////////////////////////////////////////////////////////////////////
const double volce::solver::approx_lat_count_basic(int *bools, unsigned int nRows, std::vector<int> vars){
	//nVars: the number of "decided" numeric variables
	//nRows: the number of "decided" linear formulas
	//nFormulas: the number of linear formulas
	//bools[nFormulas], vars[nVars]
	...
	//counting
	std::string filename = "alc_in.tmp";
	...
	for(unsigned int i = 0; i < nFormulas; i++) {
		...
		if ((cmp == 1 && bools[i] == 1) || (cmp == -10 && bools[i] == 0)){
			...
			ofile << (-1) * (bigb[i] + 1) << std::endl;
		}else if ((cmp == 1 && bools[i] == 0) || (cmp == -10 && bools[i] == 1)){
			//LE
			ofile << "1 ";
			for (unsigned int j = 0; j < nVars; j++)
				ofile << bigA(i, vars[j]) << " ";
			ofile << bigb[i] << std::endl;
		}else if ((cmp == 10 && bools[i] == 1) || (cmp == -1 && bools[i] == 0)){
			//GE
			ofile << "1 ";
			for (unsigned int j = 0; j < nVars; j++)
				ofile << (-1) * bigA(i, vars[j]) << " ";
			ofile << (-1) * bigb[i]  << std::endl;
		}else if ((cmp == 10 && bools[i] == 0) || (cmp == -1 && bools[i] == 1)){
			//LT
			ofile << "1 ";
			for (unsigned int j = 0; j < nVars; j++)
				ofile << bigA(i, vars[j]) << " ";
			ofile << bigb[i] - 1 << std::endl;
		}else if (cmp == 0){
			//EQ = LE + GE
			ofile << "1 ";
			for (unsigned int j = 0; j < nVars; j++)
	...
	//execute latte
	std::string cmd = tooldir + "/ApproxLatCount " + filename + " >/dev/null 2>/dev/null";
	int proc = system(cmd.c_str());
...
const double volce::solver::approx_lat_count(int *boolsol){
	std::vector<int> vars;
	unsigned int nRows = 0;
	double count = 1;
	...
	if (!enable_fact){
		...
		if (nVars_decided_total == 0)
			...
			return approx_lat_count_basic(boolsol, nRows, vars) * cube_count;
	}
	...
	for (unsigned int i = 0; i < npbools; i++){
		...
		if (nVars_decided == 1)
			...
			count *= approx_lat_count_basic(pbools[i], nRows, vars);
