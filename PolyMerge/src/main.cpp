...
void printUsage(char* exec_name){
    ...
    cout << endl;
    cout << "  -B,-b       \t   Enable Barvinok to count integer solutions. The input of" << endl;
    cout << "              \t   linear inequalities should be integers (QF_LIA logic)." << endl;
    cout << "  -A,-a       \t   Enable ALC to approximate integer solutions. The input of" << endl;
    cout << "              \t   linear inequalities should be integers (QF_LIA logic)." << endl;
    cout << endl;
...
int main(int argc, char **argv) {
    ...
    //if (!polyvest && !vinci && !barvinok && !alc) {
...
*/	
	...
	//printf("#Bunches: %d\n", count);
	while (count < 100 && s.solve())
	{
		bool newbunch = true;
		
		int *sol = s.bsols[count];
		for (unsigned int i = 0; i < s.bsols.size() - 1; i++)
		{
			bool same = true;
			int *tmps = s.bsols[i];
			for (unsigned int j = 0; j < s.nFormulas; j++)
				if (sol[j] != tmps[j]) 
				{ 
					same = false; 
					break; 
			...
			if (same)
			{
				std::cout << "Found same bunch." << endl;
				s.multiplier[i] += s.multiplier[count];
				s.bsols.pop_back();
				s.multiplier.pop_back();
				newbunch = false;
				break;
			}
		}
	...
	{
		s.merge_init();
		s.redundancy_removal_all();
		s.merge_all();
	}

	cout << "#Bunches before merge: " << count << endl;
	cout << "#Bunches after merge: " << s.bsols.size() << endl;
		
	auto t2 = Clock::now();
	
	cout << "Time cosumption of merging (sec): " << 
		(double)std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count() / 1000000000 << endl;
	...
	if (barvinok) {
		...
		for (unsigned int i = 0; i < s.bsols.size(); i++) {
	...
	if (polyvest) {
  		...
  		double *vol = new double[s.bsols.size()];
  		double maxvol = 0;
	...
	if (verbosity > 0 && (barvinok || alc || vinci || polyvest)) {	
  		cout << endl << "====================================" << endl;
  		cout << "=========== Statistics =============" << endl;
  		cout << "====================================" << endl << endl;
  		cout << "The number of bunches: " << s.bsols.size() << endl;
